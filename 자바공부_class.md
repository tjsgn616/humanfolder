...
### 람다함수
> 자바8 (2014)에 도입된 함수적 프로그래밍 기법이다. 람다가 포함되면서 자바가 완전해졌다고 말하는 이가 있을정도다.
> 원래 람다는 함수 기법인데 객체 지향 자바로 들어온 것.
...
함수형 프로그래밍에서 함수를 만들때는 side effect(부작용 : 부수효과)를 최대한 주지 않도록 구현하는데 아예없는 함수를 순수함수라고 한다. 
  ...
  그리고 함수형 프로그래밍에서는 모든것을 객체로 취급한다. 함수는 **일급객체**로 취급한다.
...
// 클래스 명 인스턴스 명 = new 클래스명();
아래는 이름이 없는 상태 = 이름이 없는 익명 객체 = 순수함수
new Object(){
  int max(int a, int b){
    return a>b? a:b;
  }
...
- 자바의 모든 메소드는 클래스 안에만 위치할수있다. 
  그래서 자바에서는 따로 메서드만 전달할 수 있는 방법이 없다. 
...
> 정의문법 :  (데이터타입 매개변수) -> { 실행문; }
              ...
              매개변수가 하나일때 괄호생략가능. 단, 타입을 명시적으로 줬다면 생략불가.
                예) (x) -> {return x*x*x;}; 이면 x -> {return x*x*X;}; 가능
...
```java
  ...
  // IntStream.range(0,10).forEach(콜백함수);
  // 자바에서는 함수만 함수의 파라미터로 전달 불가능. 그래서 위와 같이 사용.
```
- (**실제구현**)
...
- 장점 (익명 함수 - 람다식의 장점 > 자바에서 람다식은 문법상 상쇄.)
  1. 코드가 매우 간결해진다.
  2. 인스턴스 생성 안하고 기능실행
  3. 함수이름 생각 할 필요없음
